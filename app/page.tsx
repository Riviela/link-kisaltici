...
export default function Home() {
  return (
    <main className="min-h-dvh overflow-hidden bg-[var(--color-page)] px-5 py-6 sm:px-8 lg:px-10">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-[30px] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 shadow-[0_10px_30px_rgba(62,54,120,0.06)] sm:px-5">
      ...
      <section className="mx-auto grid min-h-[calc(100dvh-7rem)] max-w-7xl items-center gap-14 py-14 lg:grid-cols-[minmax(0,1fr)_minmax(28rem,0.82fr)] lg:py-20">
        ...
        <div aria-hidden="true" className="relative mx-auto w-full max-w-lg">
          <div className="absolute -left-8 top-12 h-40 w-40 rounded-[2.75rem] bg-[var(--color-accent-soft)]" />
          <div className="absolute -right-10 bottom-16 size-52 rounded-full bg-[var(--color-lime)] opacity-35" />
          <div className="relative mx-auto w-[82%] rotate-[2deg] rounded-[2.75rem] border border-[var(--color-surface)] bg-[var(--color-surface)] p-7 shadow-[0_35px_90px_rgba(62,54,120,0.2)]">
            ...
            <div className="mt-5 h-5 w-36 rounded-full bg-[var(--color-text)]" />
            <div className="mt-3 h-3 w-24 rounded-full bg-[var(--color-border-strong)]" />
            <div className="mt-9 space-y-3">
              <div className="h-16 rounded-2xl bg-[var(--color-accent-soft)]" />
              <div className="h-16 rounded-2xl bg-[var(--color-accent-soft)]" />
              <div className="h-16 rounded-2xl bg-[var(--color-accent-soft)]" />
            </div>
          </div>
